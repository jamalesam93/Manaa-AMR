...
## 2024-05-15 - Unsafe URL Sanitization
**Vulnerability:** The `isSafeUrl` function only blocked `javascript:` and `vbscript:` protocols.
**Learning:** `data:` URIs can be used to bypass XSS filters, and `file:` URIs can allow unauthorized local file access if rendered in certain contexts (like `iframe` or electron apps).
**Prevention:** Explicitly block `data:` and `file:` protocols along with script protocols in URL sanitizers.
