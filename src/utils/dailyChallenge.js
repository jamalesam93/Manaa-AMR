...
export function getDailyChallenge() {
    ...
    try {
        ...
        if (stored) {
            ...
            // Validate data type to prevent runtime errors on null/invalid structures
            if (data && typeof data === 'object' && data.date === today) {
                return data
            }
        }
    } catch (error) {
        if (import.meta.env.DEV) console.error('Error loading daily challenge:', error)
    }
...
export function setDailyChallenge(scenario, userAnswer, isCorrect) {
    ...
    try {
        ...
        if (import.meta.env.DEV) console.error('Error saving daily challenge:', error)
        return null
    }
}
...
export function getDailyChallengeStats() {
    ...
    try {
        ...
        if (import.meta.env.DEV) console.error('Error loading daily challenge stats:', error)
        return defaultStats
    }
}
